...
static int tiswrapped(Line len);
...
selnormalize(void)
{
	...
	if (sel.nb.x > i)
		sel.nb.x = i;
	if (sel.ne.x >= tlinelen(TLINE(sel.ne.x)))
		sel.ne.x = term.col - 1;
}
...
csihandle(void)
{
	...
	case 'T': /* SD -- Scroll <n> line down */
		DEFAULT(csiescseq.arg[0], 1);
		tscrolldown(term.top, csiescseq.arg[0]);		break;
